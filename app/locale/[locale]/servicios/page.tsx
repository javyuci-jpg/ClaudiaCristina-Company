    ...
    <main className="bg-[#F7F3EE] text-[#111111] font-[Inter]" role="main">
      ...
      <section className="w-full py-8 bg-[#E8E1D9]" aria-labelledby="titulo-servicios">
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Componente Services */}
          <Services servicios={servicios} locale={locale} />
        </div>
      </section>

      {/* Paquetes */}
      <section id="paquetes" className="w-full py-12 bg-[#E8E1D9]" aria-labelledby="titulo-paquetes">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {Array.isArray(paquetes) && paquetes.map((paquete, index) => (
              <div
                key={index}
              ...
              >
                {paquete.image && (
                  <img
                    src={paquete.image}
                    alt={paquete.title}
                    className="w-full h-48 object-cover rounded mb-6 transform transition-transform duration-700 hover:scale-105"
                  />
                )}
